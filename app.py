...
#Predictor part
test_data = pd.read_csv('test_data_deployed.csv')
...
drag_model = pickle.load(open('drag_trained_model_final.pickle', 'rb'))
lift_model = pickle.load(open('lift_trained_model_final.pickle', 'rb'))
time_model = pickle.load(open('time_trained_model_final.pickle', 'rb'))
    ...
    dcc.Tabs([
        dcc.Tab(label='MESHFREE Data Exploration', children=[
                       ...
                       html.Div(['Select Target:',
                                dcc.Dropdown(output_labels, output_labels[0], id='tab-1_output_dropdown')],
        ...
        dcc.Tab(label='MESHFREE Test Set Exploration', children=[
            ...
            html.Div(['Select Feature Parameter:',
                      ...
                      style={"margin-left": "225px", 'width': '30%', 'display': 'inline-block'}),
            html.Div(['Select Target:',
                      dcc.Dropdown(output_labels, output_labels[0], id='tab-2_output_dropdown')],
                      style={"margin-left": "50px",'width': '30%', 'display': 'inline-block'}),
...
def update_graph_tab1(input_column, output_label):
    if(output_label == 'Drag coefficient'):
        output_column = 'cD-value_Direct'
    elif(output_label == 'Lift coefficient'):
        output_column = 'cL-value_Direct'
    else:
        output_column = 'WallTimeTotal_h'
    ...
    if(output_column == 'cD-value_Direct'):
        ...
        fig.update_yaxes(title_text='C_D')
        ...
        fig.update_yaxes(title_text='C_L')
    elif(output_column == 'WallTimeTotal_h'):
        fig.update_yaxes(title_text='Computation time (hours)')
    return fig
...
def update_graph_tab2(input_column, output_label):
    fig = go.Figure()
...
def update_prediction(hmin, hmax, cycles, stencil, ord_grad, ord_lapl, kernel_lapl, kernel_neumann):
    ...
    time_label = str(round(time_output.iloc[0,0], 4)) + " - " + str(round(time_output.iloc[0,1], 4)) + " hours"
